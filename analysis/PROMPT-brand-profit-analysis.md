...
3. Run analysis/account_costs.sql → analysis/data/{{ACCOUNT}}_account_costs.json
   (store the settlement lines that carry no SKU as POSITIVE costs).
4. python3 analysis/build_report.py --account {{ACCOUNT}} --from {{DATE_FROM}} --to {{DATE_TO}}
5. Open the generated HTML in a browser, confirm it renders in light and dark, and commit
   the data files, the HTML and any threshold changes to the branch.
...
WHAT THE REPORT MUST SHOW
  Brand → product group → SKU (ASIN), expandable, with the whole P&L on every row.
  Brand and product group come from Supabase cogs.brand / cogs.product_group, falling back
  to the FBA fee preview brand. SKUs with activity but no listing still appear.
  A prominent cost-coverage gate: how many SKUs and how much net sales carry
  purchase_cost = 0, and the profit figure restricted to SKUs where cost IS known —
  that restricted figure is the honest headline.
  A ranked "where the profit leaks" list with a dollar figure and a concrete action per
  finding: sells at a loss, ad spend above break-even ACOS, refund rate above 5%,
  fee ratio above 40% of net sales, promotions above 10% of gross, unsellable returns,
  dead stock still accruing storage, Buy Box below 85%, missing landed cost.
...
THEN WRITE ME, in chat, no more than 15 lines:
  the headline P&L, the honest cost-covered profit, the three brands to fix first with the
  dollar figure and the one action each, and anything in the data you do not trust.
```
...
- **Cost is a Supabase job, not an analysis job.** The fastest win on every account is filling
  `purchase_cost` in `public.cogs` for the SKUs the report flags.
